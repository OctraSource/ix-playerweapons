ITEM.name = "ammo"
ITEM.description = "ammoDesc"
ITEM.category = "Weapons - Ammo"
...
function ITEM:GetItemClass()
	local fileInfo = debug.getinfo(1, "S")
	local fileName = fileInfo.source
	print("file name is " ..tostring(fileName))

	local cleanName = fileName:gsub(".lua", ""):gsub("@sh", "")
	print("clean name is "..tostring(cleanName))

	return tostring(cleanName)
end